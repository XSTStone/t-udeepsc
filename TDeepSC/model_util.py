...
def print_log_file(root_dir, ta_perform, end=None, epoch=None, batch_size=None, data_iter_step=None, data_loader=None, loss_meter=None, acc_meter=None, psnr_meter=None, lr=None, extra_len=None, extra_value=None):
    current_time = datetime.now().strftime('%Y_%m_%d_%H%M')
    log_filename = f"{root_dir}/{ta_perform}/{current_time}_training.log"
    ...
    if end is None:
        ...
        if ta_perform.startswith('imgc'):
            logging.info(f'[Epoch: {epoch+1}, Batch: {batch_size*data_iter_step}/{len(data_loader.dataset)}, Loss: {loss_meter.avg:.4f}, Acc: {acc_meter.avg*100}/100, lr: {max_lr:.4e}')
        ...
        if ta_perform.startswith('img') or ta_perform.startswith('text'):
            if ta_perform.startswith('imgc') or ta_perform.startswith('textc'):
                logging.info(f"Accuracy of the network of the {extra_len} test samples: {extra_value:.3f}")
            elif ta_perform.startswith('imgr'):
                logging.info(f"Average PSNR on the {extra_len} test samples: {extra_value:.3f}dB")
            elif ta_perform.startswith('textr'):
                logging.info(f"Average BLEU on the {extra_len} test samples: {extra_value:.3f}")
        elif ta_perform.startswith('msa'):
            logging.info(f"Accuracy of the network on the {extra_len} test samples: {extra_value*100:.3f}")
        elif ta_perform.startswith('vqa'):
            logging.info("Overall Accuracy is: %.02f" % (extra_value['overall']))
            logging.info(f"Per Answer Type Accuracy is the following:")
            for ansType in extra_value['perAnswerType']:
                logging.info("%s : %.02f" % (ansType, extra_value['perAnswerType'][ansType]))
